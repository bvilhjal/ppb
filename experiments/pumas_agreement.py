...
# code implements that repeated-learning idea on PPB's standardized scale.
#
...
#
# with ``t_val = t - t_train``.  Equation 1 is the Gaussian full-LD extension
# of the signal-dependent diagonal/off-diagonal moments in Zhao et al. (their
...
#
# Squaring a noisy validation numerator gives a positive finite-validation
# bias.  ``pumas_r2`` exposes the paper-style raw statistic and a conditional
# plug-in correction.  Within this plug-in working model the correction is
# exact for weights independent of the pseudo split; with fitted weights it is
# an approximation.  For binary phenotypes, Equation 3 is only an approximate
# and less interpretable performance measure, as Zhao et al. explicitly note.
# ---------------------------------------------------------------------------
